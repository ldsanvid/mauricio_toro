...
# X se consulta con una cadencia independiente para controlar costos.
X_CHECK_INTERVAL_SECONDS = int(
    os.getenv("X_CHECK_INTERVAL", "900")
)
_last_x_run_monotonic = None
    ...
    for client in data:
        ...
        for rss in client.get("rss") or []:
            ...
            sources.append({
                ...
                "incluir_brief": (
                    rss.get("incluir_brief", rss_id != "mauricio_toro") is True
                ),
            })
...
# V9 | Rescate del apellido "Toro" sólo dentro del RSS específico de Mauricio,
# con bloqueo de usos evidentemente animales/taurinos.
TORO_FALSE_POSITIVE_PATTERNS = (
    r"\b(?:un|el|este|ese|aquel)\s+toro\b",
    r"\btoros?\s+(?:bravo|de lidia|ganadero|bovino)\b",
    r"\b(?:ganaderia|ganadería|ganadero|ganadera|bovino|bovina|corrida|corridas|tauromaquia|taurino|taurina)\b",
)

def is_contextual_toro_rescue(article, matches):
    article_id = safe_text(article.get("article_id"))
    has_mauricio_rss_match = any(
        safe_text(match.get("article_id")) == article_id
        and safe_text(match.get("rss_id")) == "mauricio_toro"
        for match in matches
    )
    if not has_mauricio_rss_match:
        return False
    ...
    if not contains_match_term(haystack, "Toro"):
        return False

    for pattern in TORO_FALSE_POSITIVE_PATTERNS:
...
def choose_telegram_source(
    ...
    # V9: rescate contextual del apellido con bloqueo de falsos positivos obvios.
    if mauricio_source and is_contextual_toro_rescue(article, matches):
    ...
    for match in matches:
        ...
        # V9 FIX: el RSS mauricio_toro NO puede generar alerta sólo por
        # aceptar_match_rss. A esta altura ya fallaron tanto la mención
        # explícita como el rescate contextual de "Toro", así que se omite.
        if rss_id == "mauricio_toro":
            continue
...
def send_pending_telegrams(
    ...
    for article in articles:
        ...
        # V7: las alertas inmediatas de Google News quedan reservadas
        # exclusivamente para menciones directas de Mauricio Toro.
        # Los radares temáticos alimentan el brief programado, pero no
        # deben generar alertas individuales.
        if safe_text(source_cfg.get("rss_id")) != "mauricio_toro":
            print(
                "🧱 SOLO BRIEF | "
...
def process_source(
    ...
    for entry in reversed(entries):
        ...
        # V11: para el RSS específico de Mauricio, si el título/resumen no
        # confirma la mención, resolvemos la URL y revisamos el cuerpo antes
        # de cerrar la noticia como no relevante.
        pre_resolved_url = ""
        pre_article_text = ""

        if (
            source_cfg.get("rss_id") == "mauricio_toro"
            and not relevant
        ):
            pre_resolved_url = decode_google_news_url(link)

            if pre_resolved_url:
                print(
                    "🔗 URL RESUELTA PRECLASIFICACIÓN | "
                    f"{publisher or 'sin fuente'} | "
                    f"{pre_resolved_url}"
                )

                pre_article_text = fetch_article_text(
                    pre_resolved_url
                )
                ...
                if pre_article_text:
                    relevant, relevance_reason = classify_relevance(
                        ...
                        exclusions=source_cfg.get("exclusiones", []),
                        contexto_requerido=source_cfg.get(
                            "contexto_requerido", []
                        ),
                        contexto_tematico=source_cfg.get(
                        ...
                        aceptar_match_rss=False,
                        body_text=pre_article_text,
                    )

                    if relevant:
                        print(
                            "🛟 MAURICIO RESCATADO POR CUERPO | "
                            f"{relevance_reason} | {title}"
                        )
                    else:
                        print(
                            "🚫 MAURICIO NO CONFIRMADO EN CUERPO | "
                            f"{relevance_reason} | {title}"
                        )
                else:
                    print(
                        "⚠️ CUERPO NO DISPONIBLE PARA VALIDAR MAURICIO | "
                        f"{title}"
                    )
            else:
                print(
                    "⚠️ URL NO RESUELTA PARA VALIDAR MAURICIO | "
                    f"{title}"
                )
        ...
        if article_is_new:
            ...
            # Resolver la URL original al momento de ingresar una noticia
            # relevante que pueda alimentar Telegram o el brief.
            # Antes esto ocurría sólo dentro de send_pending_telegrams(),
            # por lo que las fuentes con enviar_telegram=false conservaban
            # permanentemente la URL de Google News.
            original_url = pre_resolved_url

            should_resolve_url = (
                relevant
                and (
                    source_cfg.get("enviar_telegram", True)
                    or source_cfg.get("incluir_brief", False)
                )
            )

            if should_resolve_url and not original_url:
            ...
            article = {
                ...
                "texto_articulo": pre_article_text,
        ...
        # V11: también repara falsos negativos históricos del RSS Mauricio.
        # Si todavía no hay cuerpo, intentamos obtenerlo una sola vez por corrida.
        if (
            not article_is_new
            and source_cfg.get("rss_id") == "mauricio_toro"
            and safe_text(article.get("relevante")).lower() != "true"
            and not safe_text(article.get("texto_articulo"))
        ):
            historical_url = safe_text(article.get("url_original"))

            if not historical_url:
                stored_google_url = (
                    safe_text(article.get("enlace"))
                    or link
                )
                historical_url = decode_google_news_url(
                    stored_google_url
                )
                if historical_url:
                    article["url_original"] = historical_url
                    print(
                        "🔧 URL MAURICIO HISTÓRICA REPARADA | "
                        f"{historical_url}"
                    )

            if historical_url:
                historical_body = fetch_article_text(
                    historical_url
                )
                if historical_body:
                    article["texto_articulo"] = historical_body
                    hist_relevant, hist_reason = classify_relevance(
                        safe_text(article.get("titulo")),
                        safe_text(article.get("resumen_rss")),
                        aliases=source_cfg.get("aliases", []),
                        exclusions=source_cfg.get("exclusiones", []),
                        ...
                        aceptar_match_rss=False,
                        body_text=historical_body,
                    )
                    if hist_relevant:
                        article["relevante"] = "true"
                        article["motivo_relevancia"] = hist_reason
                        print(
                            "🛟 MAURICIO HISTÓRICO RESCATADO | "
                            f"{hist_reason} | "
                            f"{article.get('titulo', '')}"
                        )

        # Reparación de registros históricos que todavía conservan
        # únicamente la URL de Google News. No usa OpenAI y deja de
        # ejecutarse para la noticia en cuanto url_original queda guardada.
        if (
            not article_is_new
            and relevant
            and not safe_text(article.get("url_original"))
        ):
            stored_google_url = (
                safe_text(article.get("enlace"))
                or link
            ...
            if (
                stored_google_url
                and "news.google.com" in stored_google_url
                and (
                    source_cfg.get("enviar_telegram", True)
                    or source_cfg.get("incluir_brief", False)
                )
            ):
                repaired_url = decode_google_news_url(
                    stored_google_url
                )

                if repaired_url:
                    article["url_original"] = repaired_url
                    print(
                        "🔧 URL HISTÓRICA REPARADA | "
                        f"{publisher or 'sin fuente'} | "
                        f"{repaired_url}"
                    )
                else:
                    print(
                        "⚠️ URL HISTÓRICA NO RESUELTA | "
                        f"{publisher or 'sin fuente'} | "
                        f"{title}"
                    )
...
def run_all_once() -> None:
    ...
    # 2. X
    # El ciclo general corre cada ~5 min, pero X sólo se consulta
    # cuando hayan transcurrido X_CHECK_INTERVAL_SECONDS (default 15 min).
    global _last_x_run_monotonic
    now_monotonic = time.monotonic()
    x_due = (
        _last_x_run_monotonic is None
        or (now_monotonic - _last_x_run_monotonic) >= X_CHECK_INTERVAL_SECONDS
    )

    if x_due:
        try:
            run_x_cycle("once")
            _last_x_run_monotonic = time.monotonic()
        ...
        remaining = max(
            0,
            int(
                X_CHECK_INTERVAL_SECONDS
                - (now_monotonic - _last_x_run_monotonic)
            ),
        )
        print(
            f"⏭️ X | todavía no corresponde | próxima revisión en ~{remaining}s"
        )

    # 3. YOUTUBE
    # Debe correr ANTES del brief para que los videos
    # detectados en este ciclo puedan entrar al mismo corte.
    try:
        run_youtube(dry_run=False)
    ...
    # 4. BRIEF TEMÁTICO
    # 08:00, 12:00 y 18:00 hora Colombia.
    # Corre al final para consumir Google News + YouTube
    # ya actualizados en este mismo ciclo.
    try:
        run_news_brief_if_due()

    except Exception as error:
        print(
            f"❌ ERROR BRIEF NOTICIAS: {error}"
        )
