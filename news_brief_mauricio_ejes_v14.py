...
MAX_NOTES = min(6, int(os.getenv("MAURICIO_BRIEF_MAX_NOTES", "6")))
...
def is_news_selection_candidate(row):
    ...
    if content_type in {"x", "post", "social"} or platform in {"x", "twitter"}:
...
- seleccion_ids puede mezclar noticias web, videos de YouTube y publicaciones de X cuando estén disponibles en el pool.
- Selecciona únicamente contenidos informativos y sustantivos; evita Facebook, Instagram y TikTok.
- Noticias, videos de YouTube y publicaciones de X compiten por los mismos lugares: no hay cuota obligatoria por formato.
...
def format_message(result, candidates, now_co):
    ...
    if result.get("seleccion_ids"):
        ...
        for idx in result["seleccion_ids"]:
            ...
            content_type = safe_text(row.get("content_type")).lower()
            platform = safe_text(row.get("platform")).lower()
            if content_type == "video" or platform == "youtube":
                marker = "VIDEO ▶️"
            elif content_type in {"x", "post", "social"} or platform in {"x", "twitter"}:
                marker = "✖️"
            else:
                marker = "•"
            if url:
    ...
    # Respeta el límite de Telegram sin cortar una referencia a la mitad.
    # Si las 6 caben, salen las 6; si no, elimina contenidos desde el final.
    message = "\n".join(lines).strip()
    while len(message) > MAX_TELEGRAM_CHARS and result.get("seleccion_ids"):
        result["seleccion_ids"] = result["seleccion_ids"][:-1]
        return format_message(result, candidates, now_co)
